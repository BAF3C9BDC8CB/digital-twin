# Qdrant 过滤与 payload 更新 API

**用途**: Qdrant 过滤条件语义与 payload 更新机制, 用于缺口补偿/数据修复类开发。

**验证基础**: 本机运行中的 Qdrant(127.0.0.1:6333, v1.18.2) REST + qdrant-client 1.18.0 crate 源码确认。复验脚本: `scripts/verify-qdrant-gap-filter.sh`。
...
## 5. Phase 2 缺口补偿机制设计- 缺陷根因：Phase 2 逐方法 LLM 分析失败只 warn 不写持久化标记（pipeline.rs L537-543），
...
## 6. 二次设计复核补充完整架构方案（时序/失败策略/幂等并发/风险边界/7 项落地清单）见
...
## 7. 全库消费方审计补充### 7.1 双写/覆盖路径——跨构建会擦除已分析内容（比"从未分析"更糟）
...
- ⚠️ `pipeline/test/runner.rs` + `test/expected.json` 已随 清理删除（test/ tests/ 目录与 build --test 一并移除），此处的 `has_llm_analysis_on_methods=true` 断言不再存在。
...
- `read_file` 会把本仓库部分 UTF-8 文本文件误判为 binary：`interfaces/cli/search_render.rs`、
  ...
  （实际无 NUL，`file` 报 Unicode text；`pipeline/test/runner.rs` 已随 清理删除）。
...
## 9. 构建层落地要点⚠️ 本节的 pipeline.rs / main.rs / builder.rs / cli-build.rs / service.rs 改动最初因工具预算中断未验证，
...
- ⚠️ main.rs 共 **3 处**调用点要加 `llm_backfill` 参数：--test 分支 handle_build、普通 handle_build、handle_build_all。

## 10. 搜索层落地

### 10.1 基础设施补充（qdrant repo.rs 新增 3 个 trait 方法）
...
### 10.4 验证- 单测：674 passed（lib）；新增 filter is_empty/is_null、noop scroll_points/set_payload、渲染占位测试全绿。
