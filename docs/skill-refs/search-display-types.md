# Search display-type mechanics — two-dimension design (file_type + content_type)

**用途**: 理解搜索结果的显示类型机制(文件类型 vs 内容类型双维度)。

## 背景

`dt search "支付"`(world=all) 曾有两个显示类型问题, 均由双维度设计(file_type + content_type)解决。

## Issue 1: yaml 文件显示为 `[Doc]` 而非 `[Config]`
