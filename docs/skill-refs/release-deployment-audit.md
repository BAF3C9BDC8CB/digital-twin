...
The MCP server calls `dt` via `subprocess` only (gRPC daemon removed); binary swap takes effect on the next CLI invocation. Restart is only needed for verified long-running dt processes using the old executable.
