...
方案演进: 初版「缺口补偿」(以 Qdrant `llm_analysis is_empty` 为事实来源, 构建末尾扫描补齐) 被否决——**不能以 llm_analysis 缺失的点为事实来源, LLM 与其他逻辑必须分开**: 前期的向量、知识图谱关系建立**不应该依赖 LLM 分析**; LLM 失败不能影响基础数据完整性。更深层动机: 旧系统的"降级"机制(llm_client.rs:157 空图占位、pipeline.rs 空响应假成功 mark)导致**有效/无效/未处理三种数据无法区分, 只能全删重建**。要的是状态可辨识。
...
状态位（Qdrant payload，定案放这里：状态跟着数据走，查询零成本）
  llm_status: "success" | "failed" | (缺失=未处理)
  llm_analysis: 文本（仅 success 时有）
  llm_retries: 失败重试计数

检索（定案：base 只召回，llm 做 rerank）
...
- ⚠️ **存量单向量集合 collection_exists=true 不重建** → 换新代码必须删集合重建（ 已授权全量删）
...
- **补偿自愈 backfill_llm_gaps**（Phase 2 await 之后、BuildReport 之前）：
  ...
  - ⚠️ **必须并发**：初版是串行 `for point in ...` 循环——一次只发 1 个 LLM 请求，opencode.go 平均 12.8s/个 → 302 个缺口 ≈ 64 分钟，且构建日志表现为"卡在补偿阶段"（大量 `补偿成功` 但无 Phase 2 日志）。改为 `stream::iter(...).map(|point| async move {...}).buffer_unordered(self.phase2_concurrency)`（与 Phase 2 主循环同一并发值），闭包返回 `(u64, Option<bool>)`（Some(true)=成功/Some(false)=失败/None=跳过），末尾聚合 succeeded/failed。改造后 16 路并发 ≈ 4 分钟，16 倍加速。**新代码的补偿/批量 LLM 循环一律 buffer_unordered，禁止串行 await。**
...
- 存量数据全量删除重建（ 已授权）：重建途中的 LLM 失败会被补偿机制自动补齐
...
## ⚠️ ensure_collection 双向量副作用坑**症状**：warehouse-center 构建日志 `GPU 处理器执行失败 processor="store" path=.gitlab-ci.yml error=... Not existing vector name`。
...
## Qdrant named vectors API 实测（1.18.2 / qdrant-client 1.18.0， ）
