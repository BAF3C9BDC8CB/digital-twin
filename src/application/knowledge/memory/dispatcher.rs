...
use super::handlers::{
    BugFixHandler, ConfigChangeHandler, ConversationHandler,
...
pub fn build_default_dispatcher() -> EventDispatcher {
    ...
    d.register(Box::new(BugFixHandler));
...
mod tests {
    ...
    #[tokio::test]
    async fn default_dispatcher_has_six_handlers() {
    ...
    #[tokio::test]
    async fn default_dispatcher_routes_decision() {
        let d = build_default_dispatcher();
        let repo = MockRepo;
        let evt = make_event(EventType::Decision);
        d.dispatch(&evt, &repo).await.expect("dispatch should succeed");
    }
