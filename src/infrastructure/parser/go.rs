...
fn extract_calls(body: &str) -> Vec<String> {
    ...
    let mut calls: Vec<String> = re.captures_iter(body).filter_map(|c| {
        ...
        if matches!(name.as_str(), "if" | "for" | "return" | "range" | "switch" | "case" | "default" | "defer" | "go" | "select" | "chan" | "make" | "new" | "append" | "len" | "cap" | "func" | "type" | "var" | "const") { None }
        else { Some(name) }
    }).collect();
    ...
    fn parse(&self, source: &str, path: &Path, project: &str) -> Result<ParseResult, DtError> {
        ...
        for caps in struct_re.captures_iter(source) {
            ...
            classes.push(ClassBlock {
                class_id, name, kind: ClassKind::Struct,
        ...
        for caps in interface_re.captures_iter(source) {
            ...
            let class_id = make_class_id(project, &module, &name);
            classes.push(ClassBlock {
                class_id, name, kind: ClassKind::Interface,
                file_path: file_path.clone(), package_or_module: module.clone(),
                project: project.to_string(), start_line, end_line: 0, method_ids: Vec::new(),
            });
        }
        ...
        for caps in func_re.captures_iter(source) {
            let receiver_name = caps.get(1).map(|m| m.as_str().to_string()).unwrap_or_default();
            let receiver_type = caps.get(2).map(|m| m.as_str().to_string()).unwrap_or_default();
            let func_name = caps[3].to_string();
            let params = caps[4].to_string();
            let return_type = caps.get(5).map(|m| m.as_str().trim().to_string()).unwrap_or_default();
            ...
            let current_class = if !receiver_type.is_empty() {
                ...
                classes.iter().filter(|c| c.start_line <= start_line && c.end_line >= start_line)
            ...
            let signature = if !receiver_type.is_empty() {
                format!("func ({} {}) {}({}) {}", receiver_name, receiver_type, func_name, params, return_type)
            } else {
                format!("func {}({}) {}", func_name, params, return_type)
            };

            methods.push(MethodBlock {
                method_id, name: func_name, signature, params,
...
mod tests {
    ...
    fn parses_function_and_struct() {
        ...
        let result = GoParser.parse(src, &PathBuf::from("main.go"), "test").unwrap();
