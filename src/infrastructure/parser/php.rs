...
fn extract_calls(body: &str) -> Vec<String> {
    ...
    let mut calls: Vec<String> = re.captures_iter(body).filter_map(|c| {
        ...
        if matches!(name.as_str(), "if" | "for" | "foreach" | "while" | "return" | "throw"
            | "new" | "echo" | "print" | "isset" | "empty" | "unset" | "include"
            | "require" | "namespace" | "use" | "function" | "class" | "interface"
            | "trait" | "catch" | "try" | "finally" | "switch" | "case" | "default") { None }
        else { Some(name) }
    }).collect();
...
fn extract_namespace(source: &str) -> String {
    ...
    re.captures(source).map(|c| c[1].to_string()).unwrap_or_default()
}
...
impl ParseStrategy for PhpParser {
    ...
    fn parse(&self, source: &str, path: &Path, project: &str) -> Result<ParseResult, DtError> {
        ...
        let class_re = Regex::new(r"(?m)^\s*(abstract\s+)?(class|interface|trait)\s+(\w+)\s*[^{]*\{").unwrap();
        ...
        for caps in func_re.captures_iter(source) {
            ...
            methods.push(MethodBlock {
                method_id, name, signature, params, return_type: "mixed".into(),
...
mod tests {
    ...
    fn parses_class_and_method() {
        ...
        let result = PhpParser.parse(src, &PathBuf::from("Hello.php"), "test").unwrap();
