...
impl ParseStrategy for TsPhpParser {
    ...
    fn can_parse(&self, path: &Path) -> bool {
        path.extension().and_then(|e| e.to_str()).map(|e| e == "php").unwrap_or(false)
    }
    fn parse(&self, source: &str, path: &Path, project: &str) -> Result<ParseResult, DtError> {
        ...
        parser.set_language(&lang).map_err(|e| DtError::Repository(format!("ts php init: {e}")))?;
        let tree = parser.parse(source, None).ok_or_else(|| DtError::Repository("ts php parse failed".into()))?;
        ...
        // Collect classes
        {
            ...
            for ch in root.children(&mut c) {
                if matches!(ch.kind(), "class_declaration" | "interface_declaration" | "trait_declaration") {
                    if let Some(nn) = ch.child_by_field_name("name") {
                        ...
                        classes.push(ClassBlock {
                            ...
                            name, kind, file_path: file_path.clone(), package_or_module: ns.clone(),
        ...
        fn collect_methods_from(source: &str, node: &tree_sitter::Node, project: &str, file_path: &str, ns: &str, class_name: &str) -> Vec<MethodBlock> {
            let mut methods = Vec::new();
            let mut c = node.walk();
                ...
                if ch.kind() == "method_declaration" {
                    if let Some(nn) = ch.child_by_field_name("name") {
                        ...
                        methods.push(MethodBlock {
                            ...
                            file_path: file_path.to_string(), package_or_module: ns.to_string(),
        ...
        for cls in &classes {
            let mut c = root.walk();
            let _ = find_php_class_methods(source, &root, &mut c, project, &file_path, &ns, &cls.name, &mut methods);
        }
                ...
                if ch.kind().contains("function_definition") && !is_inside_class(&root, &ch) {
                    if let Some(nn) = ch.child_by_field_name("name") {
                        ...
                        methods.push(MethodBlock {
                            ...
                            name, signature: sig, params: params_clean, return_type: "mixed".into(),
...
fn is_inside_class(root: &tree_sitter::Node, node: &tree_sitter::Node) -> bool {
    ...
    for ch in root.children(&mut c) {
        if matches!(ch.kind(), "class_declaration" | "interface_declaration" | "trait_declaration") {
            ...
            if start > cs && start <= ce { return true; }
        }
    }
    false
}
...
fn find_php_class_methods(
    ...
    project: &str, file_path: &str, ns: &str, class_name: &str, methods: &mut Vec<MethodBlock>,
) {
    loop {
            ...
            if let Some(nn) = node.child_by_field_name("name") {
                if tree_sitter_utils::node_text(source, &nn) == class_name {
                    if let Some(body) = node.child_by_field_name("body") {
                        let ms = collect_methods_from(source, &body, project, file_path, ns, class_name);
                        methods.extend(ms);
                    }
                    return;
                }
