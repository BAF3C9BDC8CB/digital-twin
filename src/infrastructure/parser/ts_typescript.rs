...
impl TsTypeScriptParser {
    fn collect_classes(source: &str, node: &tree_sitter::Node, project: &str, file_path: &str, module: &str, classes: &mut Vec<ClassBlock>) {
        // Also look inside export_statement for class/interface/enum declarations
        let target = if matches!(node.kind(), "class_declaration" | "interface_declaration" | "enum_declaration") {
            Some(*node)
        } else if node.kind() == "export_statement" {
            node.named_children(&mut node.walk())
                .find(|ch| matches!(ch.kind(), "class_declaration" | "interface_declaration" | "enum_declaration"))
        } else {
            None
        };
        ...
        let mut c = node.walk();
        for ch in node.children(&mut c) { Self::collect_classes(source, &ch, project, file_path, module, classes); }
    }

    fn build_method(source: &str, node: &tree_sitter::Node, project: &str, file_path: &str, module: &str, class_name: &str, name: &str, sl: usize, el: usize, params: &str, sig: &str, body: &str, calls: Vec<String>) -> MethodBlock {
        MethodBlock {
            method_id: make_method_id(project, file_path, class_name, name, sl),
            name: name.to_string(), signature: sig.to_string(), params: params.to_string(),
            return_type: "any".into(), class_name: class_name.to_string(),
            file_path: file_path.to_string(), package_or_module: module.to_string(),
            language: "typescript".into(), project: project.to_string(),
            start_line: sl, end_line: el, calls, comment: String::new(), source_text: body.to_string(),
        }
    }

    /// Extract a function from a node that may be function_declaration or export_statement wrapping one.
    fn extract_fn(source: &str, node: &tree_sitter::Node) -> Option<(String, usize, usize, String, String, String, Vec<String>)> {
        ...
        target.and_then(|fn_node| {
            ...
            let sig = tree_sitter_utils::node_text(source, &fn_node).lines().next().unwrap_or("").trim().to_string();
...
impl ParseStrategy for TsTypeScriptParser {
    ...
    fn can_parse(&self, path: &Path) -> bool {
        path.extension().and_then(|e| e.to_str()).map(|e| matches!(e, "ts" | "tsx")).unwrap_or(false)
    }
    fn parse(&self, source: &str, path: &Path, project: &str) -> Result<ParseResult, DtError> {
        ...
        let is_tsx = path.extension().and_then(|e| e.to_str()).map(|e| e == "tsx").unwrap_or(false);
        ...
        {
            let mut c = root.walk();
            for ch in root.children(&mut c) { Self::collect_classes(source, &ch, project, &file_path, &module, &mut classes); }
        }
        ...
        // Top-level functions (including those inside export_statement)
        {
            ...
            for ch in root.children(&mut c) {
                ...
                if let Some((name, sl, el, params, sig, body, calls)) = Self::extract_fn(source, &ch) {
                    methods.push(Self::build_method(source, &ch, project, &file_path, &module, "_module_", &name, sl, el, &params, &sig, &body, calls));
                }

                // Arrow functions (including those inside export_statement)
                if ch.kind() == "export_statement" {
                    let mut tmp_c = ch.walk();
                    let container = ch.children(&mut tmp_c).find(|c| matches!(c.kind(), "lexical_declaration" | "variable_declaration"));
                    if let Some(container) = container {
                        let mut dc = container.walk();
                        for decl in container.children(&mut dc) {
                            if decl.kind() == "variable_declarator" {
                                Self::extract_arrow_fn(source, &decl, project, &file_path, &module, &mut methods);
                            }
                        }
                    }
        ...
        for cls in &classes {
            let mut c = root.walk();
            find_ts_class_methods(source, &root, &mut c, project, &file_path, &module, &cls.name, &mut methods);
        }
...
impl TsTypeScriptParser {
    fn get_params(source: &str, node: &tree_sitter::Node) -> String {
        if let Some(p) = node.child_by_field_name("parameters") {
            ...
            if t.len() >= 2 { t[1..t.len()-1].to_string() } else { String::new() }
        } else { String::new() }
    }
    ...
    fn extract_arrow_fn(source: &str, decl: &tree_sitter::Node, project: &str, file_path: &str, module: &str, methods: &mut Vec<MethodBlock>) {
        if let Some(init) = decl.child_by_field_name("value") {
            if init.kind().contains("arrow") || init.kind().contains("function") {
                if let Some(nn) = decl.child_by_field_name("name") {
                    ...
                    methods.push(Self::build_method(source, &init, project, file_path, module, "_module_", &name, sl, el, &params, &sig, &body, calls));
                }
            }
        }
    }
...
fn find_ts_class_methods(
    source: &str, _root: &tree_sitter::Node, cursor: &mut tree_sitter::TreeCursor,
    project: &str, file_path: &str, module: &str, class_name: &str, methods: &mut Vec<MethodBlock>,
) {
    loop {
                        ...
                        for member in body.children(&mut bc) {
                            if member.kind() == "method_definition" {
                                if let Some(mn) = member.child_by_field_name("name") {
                                    let name = tree_sitter_utils::node_text(source, &mn).to_string();
                                    let (sl, el) = tree_sitter_utils::node_range(&member);
                                    let params = TsTypeScriptParser::get_params(source, &member);
                                    let sig = tree_sitter_utils::node_text(source, &member).lines().next().unwrap_or("").trim().to_string();
                                    let body_text = TsTypeScriptParser::get_body(source, &member);
                                    let calls = tree_sitter_utils::extract_calls_from_body(source, &member);
                                    methods.push(TsTypeScriptParser::build_method(source, &member, project, file_path, module, class_name, &name, sl, el, &params, &sig, &body_text, calls));
                                }
                            }
                        }
                    }
