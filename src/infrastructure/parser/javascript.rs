...
fn extract_calls(body: &str) -> Vec<String> {
    ...
    let mut calls: Vec<String> = re.captures_iter(body).filter_map(|c| {
        let name = c[1].to_string();
        if matches!(name.as_str(), "if" | "for" | "while" | "switch" | "return" | "throw"
            | "new" | "typeof" | "instanceof" | "catch" | "try" | "await" | "async"
            | "import" | "export" | "from" | "default" | "break" | "continue" | "function"
            | "class" | "var" | "let" | "const" | "require" | "module") { None }
        else { Some(name) }
    }).collect();
    calls.sort(); calls.dedup(); calls.truncate(50);
    calls
}
...
impl ParseStrategy for JavaScriptParser {
    ...
    fn parse(&self, source: &str, path: &Path, project: &str) -> Result<ParseResult, DtError> {
        ...
        let class_re = Regex::new(r"(?m)^\s*(export\s+)?class\s+(\w+)\s*(extends\s+\w+)?\s*\{").unwrap();
        let func_re = Regex::new(r"(?m)^\s*(export\s+)?(async\s+)?function\s+(\w+)\s*\(([^)]*)\)\s*\{").unwrap();
        let arrow_re = Regex::new(r"(?m)^\s*(export\s+)?(const|let|var)\s+(\w+)\s*=\s*(async\s+)?\([^)]*\)\s*=>\s*\{").unwrap();
        let method_re = Regex::new(r"(?m)^\s*(static\s+)?(async\s+)?(\w+)\s*\(([^)]*)\)\s*\{").unwrap();
        ...
        for caps in class_re.captures_iter(source) {
            ...
            classes.push(ClassBlock {
                class_id, name, kind: ClassKind::Class,
        ...
        for caps in func_re.captures_iter(source) {
            ...
            methods.push(MethodBlock {
                ...
                params, return_type: "any".into(), class_name: "_module_".into(),
                file_path: file_path.clone(), package_or_module: module.clone(),
        ...
        for caps in arrow_re.captures_iter(source) {
            ...
            methods.push(MethodBlock {
                ...
                params: "...".into(), return_type: "any".into(), class_name: "_module_".into(),
                file_path: file_path.clone(), package_or_module: module.clone(),
                language: "javascript".into(), project: project.to_string(),
                start_line, end_line: start_line + body.lines().count(),
                calls, comment: String::new(), source_text: body,
        ...
        for caps in method_re.captures_iter(source) {
            ...
            let params = caps[4].to_string();
            if method_name == "class" || method_name == "function" || method_name == "if" { continue; }
            let start_byte = caps.get(0).unwrap().start();
            let start_line = source[..start_byte].lines().count() + 1;
            ...
            methods.push(MethodBlock {
                ...
                params, return_type: "any".into(), class_name: current_class,
                file_path: file_path.clone(), package_or_module: module.clone(),
                language: "javascript".into(), project: project.to_string(),
                start_line, end_line: start_line + body.lines().count(),
                calls, comment: String::new(), source_text: body,
...
mod tests {
    ...
    fn parses_function_and_class() {
        ...
        let result = JavaScriptParser.parse(src, &PathBuf::from("app.js"), "test").unwrap();
