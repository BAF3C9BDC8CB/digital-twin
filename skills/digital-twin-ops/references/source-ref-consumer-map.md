# source_ref 消费方地图(2026-08-11 全库审计)

适用:任何改 source_ref / doc_id / 搜索「来源」显示的变更前必读。审计方式:`rg -n "source_ref" src/`(60 处)+ `rg -n "dt://" src/` + mcp 侧核查。
...
| 位置 | 用途 | 值替换影响 |
|---|---|---|
| search_render.rs:66-67 | render_human `来源: {source_ref}`(用户要改的显示行) | 显示层改动点(方案 A) |
...
1. **仅显示层(render_human `来源:` 行)= 低**:JSON/MCP 不变;「来源」是用户契约级元素,渲染断言 search_render.rs:164-168/193-198/232 需同步;JSON 与人类输出不一致。
