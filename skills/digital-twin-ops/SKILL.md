...
**Three data paths** — file builds (`dt build`, pipeline with hash-incremental + LLM extraction) vs Nacos sync (`dt nacos-sync`, SyncSource → `config_chunks`) vs Jenkins sync (`dt jc-sync` / `jcli build` auto-sync, SyncSource → no vectors). Full code-level walkthrough (entry points, node ids, relations, diff table): `references/sync-vs-build-architecture.md`.
