...
**缺口盘点三步法(只读)**: ① 点数 vs 分析数对比(`points/count` filter project vs `build_progress` count) → 差距=缺口规模; ② 排除空 hash 假完成根因(`file_sha1='e3b0c44...'` 为 0 → 非接口空 source_text 根因); ③ 日志时间线定根因(`grep '<文件>' dt.log`: 只有 `ProcessorEngine CPU start/done` 无 `StoreProcessor start` 也无 `Phase 2 ... 已写入` = 从未走到 Phase 2)。交叉比对注意 **build_progress.file_path 存 `method:{entity_id}` 而非源文件路径**。一键审计: `scripts/llm_gap_audit.py`(只读, 输出 /tmp/dt_llm_gap_report.json)。
