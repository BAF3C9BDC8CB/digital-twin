...
hermes setup
​
如果你在设置向导中选择了 NeuTTS，Hermes 会检查
neutts
是否已安装。如果缺失，向导会告知你 NeuTTS 需要 Python 包
neutts
和系统包
espeak-ng
，并提供自动安装，使用平台包管理器安装
...
（默认）会立即提交转写文本。
draft
会把转写文本放入输入框，供你编辑或取消，按 Enter 后才发送。
如需可编辑的语音草稿，请设置：
voice
:
submit_mode
...
默认情况下，除非另行配置，机器人在 Discord 服务器文本频道中需要被
@mention
才会响应。
建议的第一周方案
​
