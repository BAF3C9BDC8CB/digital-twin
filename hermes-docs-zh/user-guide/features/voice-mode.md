...
付费的
Nous Portal
订阅通过 Tool Gateway 同时提供 LLM（第 2 步）
和
OpenAI TTS — 无需单独的 OpenAI 密钥。全新安装时，
hermes setup --portal
可一次性完成两项配置。
概览
​
...
交互式语音
CLI
按 Ctrl+B 开始录音，Agent 自动检测静音并回复
自动语音回复
Telegram、Discord
Agent 在文字回复的同时发送语音音频
语音频道
Discord
Bot 加入语音频道，监听用户发言并语音回复
环境要求
​
Python 包
​
...
（用于语音加密）和
opus 绑定
。这是 Discord 语音频道支持的必要条件。
系统依赖
​
# macOS
brew
install
portaudio ffmpeg opus
brew
install
espeak-ng
# for NeuTTS
...
apt
install
portaudio19-dev ffmpeg libopus0
sudo
apt
install
espeak-ng
...
=
your-key
# Groq Whisper — 速度快，有免费额度（云端）
VOICE_TOOLS_OPENAI_KEY
=
your-key
# OpenAI Whisper — 付费（云端）
...
如果已安装
faster-whisper
，语音模式的 STT 无需任何 API 密钥即可运行。模型（
base
约 150 MB）会在首次使用时自动下载。
CLI 语音模式
​
语音模式在
经典 CLI
...
（
hermes --tui
）中均可使用。两者行为完全一致 — 相同的斜杠命令、相同的 VAD（语音活动检测）静音检测、相同的流式 TTS、相同的幻觉过滤器。TUI 额外将崩溃诊断日志转发至
~/.hermes/logs/
，以便在异常音频后端出现按键录音失败时提供完整堆栈跟踪，而非静默消失。
快速开始
​
启动 CLI 并启用语音模式：
hermes
...
两阶段算法检测您是否已停止说话：
语音确认
— 等待音频 RMS 值超过阈值（200）至少 0.3 秒，允许音节间的短暂停顿
结束检测
— 语音确认后，持续静音 3.0 秒即触发停止
...
流式 TTS
​
启用 TTS 后，Agent 在生成文字的同时
逐句
朗读回复 — 无需等待完整响应：
将文字增量缓冲为完整句子（最少 20 个字符）
去除 Markdown 格式和
<think>
块
实时逐句生成并播放音频
...
Bot 必须被邀请到服务器
私信（个人使用推荐）：
直接与 Bot 开启私信并发送消息 — 无需 @提及。语音回复和所有命令与在频道中使用完全相同。
服务器频道：
Bot 仅在被 @提及时响应（例如
@hermesbyt4 你好
）。请确保从提及弹窗中选择
...
Telegram
语音气泡（Opus/OGG）
在聊天中内联播放。如需要，ffmpeg 将 MP3 转换为 Opus
Discord
原生语音气泡（Opus/OGG）
像用户语音消息一样内联播放。如语音气泡 API 失败则回退为文件附件
Discord 语音频道
​
...
仅文字
309237763136
查看频道、发送消息、读取历史、嵌入内容、附件、帖子、反应、创建公开帖子
文字 + 语音
309240908864
以上所有 + Connect、Speak
...
仅在
DISCORD_ALLOWED_USERS
列表使用用户名时才需要 — 如果使用数字用户 ID，可以关闭。语音频道中 SSRC → user_id 的映射来自 Discord 语音 WebSocket 上的 SPEAKING opcode，
不
需要 Server Members Intent。
3. Opus 编解码器
​
...
运行
/voice join
前，您必须已在某个语音频道中。Bot 会加入您所在的语音频道。
工作原理
​
Bot 加入语音频道后：
独立监听
每位用户的音频流
检测静音
— 至少 0.5 秒语音后出现 1.5 秒静音即触发处理
转录
音频（通过本地、Groq 或 OpenAI 的 Whisper STT）
处理
完整的 Agent 流水线（会话、工具、记忆）
语音回复
通过 TTS 在语音频道中播放
文字频道集成
​
...
只有
DISCORD_ALLOWED_USERS
中列出的用户才能通过语音进行交互。其他用户的音频会被静默忽略。
# ~/.hermes/.env
DISCORD_ALLOWED_USERS
=
284102345871466496
...
:
"local"
# "local"（免费）| "groq" | "openai" | "mistral" | "xai"
local
:
model
:
...
:
"edge"
# "edge"（免费）| "elevenlabs" | "openai" | "neutts" | "minimax" | "mistral" | "gemini" | "xai" | "kittentts" | "piper"
edge
:
voice
:
